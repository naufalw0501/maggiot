...
const App = () => {
  ...
  async function getToken() {
    ...
    const tokenPathtotaldevice = `sistem1/devicetoken/totaldevice`;
    const snapshot2 = await database().ref(tokenPathtotaldevice).once("value");
    const deviceNumberex = snapshot2.val();
    const deviceNumber = deviceNumberex + 1;
    
    const tokenPath = `sistem1/devicetoken/device${deviceNumber}`;
    const tokenPathCheck = `sistem1/devicetoken`;
    const snapshot = await database().ref(tokenPathCheck).once("value");
    const tokenValues = snapshot.val();
  
    if (tokenValues && Object.values(tokenValues).includes(fcmToken)) {
      console.log('Device Token already exists');
    } else {
      await database().ref(tokenPath).set(fcmToken);
      await database().ref(tokenPathtotaldevice).set(deviceNumber);
